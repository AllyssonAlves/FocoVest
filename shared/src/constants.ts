...
export const ACHIEVEMENT_REQUIREMENTS = {
  ...
  SPEED_DEMON: { avgTime: 30 }, // seconds per question
...
export const API_ENDPOINTS = {
  ...
  SIMULATIONS: {
    ...
    START: (id: string) => `/api/simulations/${id}/start`,
    SUBMIT: (id: string) => `/api/simulations/${id}/submit`,
    RESULTS: (id: string) => `/api/simulations/${id}/results`
  },
  RANKINGS: {
    GENERAL: '/api/rankings/general',
    BY_UNIVERSITY: (university: University) => `/api/rankings/university/${university}`,
    BY_SUBJECT: (subject: Subject) => `/api/rankings/subject/${subject}`
  }
}
